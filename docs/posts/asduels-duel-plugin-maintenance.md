---
title: "AsDuels: Maintaining a Minecraft Duel Plugin | AsDuels：一个决斗插件的二次开发维护"
date: 2026-08-10 21:00:00
tags: [Minecraft, Java, Development, Plugin]
---

> 中文版在下文（English first, Chinese below）.
...
```java
match.setState(MatchState.ENDING);
plugin.getPointsManager().onMatchEnd(winner, loser); // 可能抛异常！
finishMatchCleanup(match, lobby); // 一旦抛异常，这行永远到不了
```
...
try {
    ...
    plugin.getLogger().warning("积分结算出错：" + ex.getMessage());
} finally {
    forceCleanup(match); // 保证清理
}
```
